...
<section class="quote-section white-text text-center <?php echo $testimonial_bg; ?> bg">
	<div class="container">
		...
		if ( get_sub_field( 'testimonial_title' ) ) {
			?>
			<h2 class="<?php echo $testimonial_title_color; ?>">"<?php the_sub_field( 'testimonial_title' ); ?>"</h2><?php } ?>
		<div class="default-content">
			<?php
			if ( get_sub_field( 'testimonial_description' ) ) {
				?>
				<p><?php the_sub_field( 'testimonial_description' ); ?></p><?php } ?>
		</div>
		<?php
		if ( get_sub_field( 'author_name' ) ) {
			?>
			<p class="author-name"><?php the_sub_field( 'author_name' ); ?></p><?php } ?>
	</div>
</section>
